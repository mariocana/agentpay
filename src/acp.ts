...
export async function fund(wallet: Wallet, jobId: bigint): Promise<Hex> {
  ...
  const allowance = await publicClient.readContract({ address: USDC, abi: erc20Abi, functionName: "allowance", args: [owner, ERC8183] });
  if (allowance < job.budget) {
    await send(wallet, "approve", () =>
      wallet.writeContract({ address: USDC, abi: erc20Abi, functionName: "approve", args: [ERC8183, job.budget] }),
    );
  }
